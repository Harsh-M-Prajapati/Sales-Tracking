...
export class LoginService{
    ...
    url='https://apisales.azurewebsites.net/api/';
    ...
    userLogin(email:string,password:string):Observable<ResponseLogin>{
        return this.http.get<ResponseLogin>(this.url+"employee/login/"+email+"/"+password).
        pipe(catchError(this.handleError));
    }

    userRegister(user:User):Observable<ResponseMessage>{
        return this.http.post<ResponseMessage>(this.url+"employee/register",user).pipe(catchError(this.handleError));
    }
